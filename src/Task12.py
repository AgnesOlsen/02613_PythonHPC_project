from os.path import join
import sys
import numpy as np
from numba import jit
from numba import cuda
import matplotlib.pyplot as plt
import time
import pandas as pd

def load_data(load_dir, bid):
    SIZE = 512
    u = np.zeros((SIZE + 2, SIZE + 2))
    u[1:-1, 1:-1] = np.load(join(load_dir, f"{bid}_domain.npy"))
    interior_mask = np.load(join(load_dir, f"{bid}_interior.npy"))
    return u, interior_mask


@cuda.jit
def jacobi_cuda_kernel(u, u_new, interior_mask):
    i, j = cuda.grid(2)
    # Interior indices
    if 1 <= i < u.shape[0] - 1 and 1 <= j < u.shape[1] - 1:
        # Do it only for the mask
        if interior_mask[i-1,j-1]:
            u_new[i, j] = 0.25 * (
                    u[i, j - 1] +  
                    u[i, j + 1] +  
                    u[i - 1, j] +  
                    u[i + 1, j]    
                )
        else:
            u_new[i, j] = u[i, j] 

def jacobi_helper(u,interior_mask,iter):
    tpb = (32,32)
    bpg = (u.shape[0]// tpb[0], 
           u.shape[1] // tpb[1])

    d_u = cuda.to_device(u)
    d_u_new = cuda.device_array_like(d_u)
    d_mask = cuda.to_device(interior_mask)
    for i in range(iter):
        jacobi_cuda_kernel[bpg, tpb](d_u, d_u_new, d_mask)
        d_u, d_u_new = d_u_new, d_u
    
    return d_u.copy_to_host()

def summary_stats(u, interior_mask):
    u_interior = u[1:-1, 1:-1][interior_mask]
    mean_temp = u_interior.mean()
    std_temp = u_interior.std()
    pct_above_18 = np.sum(u_interior > 18) / u_interior.size * 100
    pct_below_15 = np.sum(u_interior < 15) / u_interior.size * 100
    return {
        'mean_temp': mean_temp,
        'std_temp': std_temp,
        'pct_above_18': pct_above_18,
        'pct_below_15': pct_below_15,
    }

#### Have to comopile kernel 1 time before timing - so we need to load building ID's etc to test. 

LOAD_DIR = '/dtu/projects/02613_2025/data/modified_swiss_dwellings/'
with open(join(LOAD_DIR, 'building_ids.txt'), 'r') as f:
    building_ids = f.read().splitlines()
building_ids = building_ids[:1]

all_u0 = np.empty((1, 514, 514))
all_interior_mask = np.empty((1, 512, 512), dtype='bool')
for i, bid in enumerate(building_ids):
    u0, interior_mask = load_data(LOAD_DIR, bid)
    all_u0[i] = u0
    all_interior_mask[i] = interior_mask

# Run jacobi iterations for each floor plan
ITER = 5000

all_u = np.empty_like(all_u0)
for i, (u0, interior_mask) in enumerate(zip(all_u0, all_interior_mask)):
    u = jacobi_helper(u0, interior_mask, ITER)
    all_u[i] = u


if __name__ == '__main__':
    # Load data - we also time this to accurately compare to reference. 
    
    LOAD_DIR = '/dtu/projects/02613_2025/data/modified_swiss_dwellings/'
    with open(join(LOAD_DIR, 'building_ids.txt'), 'r') as f:
        building_ids = f.read().splitlines()

    if len(sys.argv) < 2:
        N = 1
    else:
        N = int(sys.argv[1])
    building_ids = building_ids[:N]

    ITER = 20_000 # Run jacobi iterations for each floor plan

    output_file = "stats/summary_statistics_final.csv"
    stat_keys = ['mean_temp', 'std_temp', 'pct_above_18', 'pct_below_15']

    # write header once
    pd.DataFrame(columns=['building_id'] + stat_keys).to_csv(output_file, index=False)

    start_time = time.time()
    jacobi_time = 0
    for bid in building_ids:
        u0, interior_mask = load_data(LOAD_DIR, bid) # Load floor plans
        jacobi_time_temp = time.time()
        u = jacobi_helper(u0, interior_mask, ITER) # run jacobi iteration
        jacobi_time += time.time() - jacobi_time_temp
        #add stats to the current batch rows
        stats = summary_stats(u, interior_mask)
        row = {'building_id': bid}
        for k in stat_keys:
            row[k] = stats[k]

        # append building to file
        df_batch = pd.DataFrame([row])
        df_batch.to_csv(output_file, mode='a', header=False, index=False)

        #explicitly make sure that the previous building data is deleted 
        del row, df_batch, u0, u, interior_mask

    total_time = time.time()-start_time
    print(f"Time for {N} is {total_time}")