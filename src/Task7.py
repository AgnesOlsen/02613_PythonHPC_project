...
def jacobi(u, interior_mask, max_iter, atol=1e-6):
    ...
    for i in range(max_iter):
        ...
        u_new_interior = np.zeros_like(u_new)
        ...
        for i in range(u_new.shape[0]): # går gennem alle rækkerne
            for j in range(u_new.shape[1]): # går gennem hvert element i rækkerne
                if interior_mask[i,j]:
                    u_new_interior[i,j] = u_new[i,j]
                    delta[i,j] = u[1:-1, 1:-1][i,j] - u_new_interior[i,j]
                    u[1:-1, 1:-1][i,j] = u_new_interior[i,j]
